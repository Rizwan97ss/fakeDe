...
Evidence AiGeneratedModelAnalyzer::analyze(const AnalysisInput& input) const {
    ...
    if (!session_->isLoaded()) {
        ...
        evidence.explanation = "AI-image classifier model is not loaded (weights not fetched).";
        return evidence;
    }
    ...
    if (bgr.empty()) {
        evidence.score = 0.5;
        evidence.confidence = 0.0;
        evidence.explanation = "Image could not be decoded for classification.";
        return evidence;
    }
    ...
    evidence.explanation = evidence.score > 0.6
        ? "The AI-image classifier assigns a high probability that this image was generated by a diffusion "
          "or GAN model."
        : "The AI-image classifier assigns a low probability of AI generation.";
